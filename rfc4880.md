...
the defining specification.  The initial values for this registry can
be found in {{user-attribute-packet}}.  Adding a new User Attribute type MUST be
...
subpackets as discussed in {{signature-subpacket}}.  This specification
...
this registry can be found in {{signature-subpacket}}.  Adding a new
...
found in {{key-server-preferences}}.  Adding a new key server preference MUST
...
the defining specification.  The initial values for this registry can
be found in {{reason-for-revocation}}.  Adding a new feature flag MUST be done
through the IETF CONSENSUS method, as described in [RFC2434].
...
initial values for this registry can be found in {{features-subpacket}}.
Adding a new feature-implementation flag MUST be done through the
IETF CONSENSUS method, as described in [RFC2434].

Also see {{meta-considerations-for-expansion}} for more information about
when feature flags are needed.
...
can be found in {{packet-types}}.  Adding a new packet version MUST be done
through the IETF CONSENSUS method, as described in [RFC2434].

## New Algorithms

{{constants}} lists the core algorithms that OpenPGP uses.  Adding in a
...
initial values for this registry can be found in {{pubkey-algos}}.  Adding a
...
initial values for this registry can be found in {{symmetric-algos}}.  Adding a
...
{{version-three-sig}} for the OIDs and expanded signature prefixes.  Adding a new
...
reference to the defining specification.  The initial values for this
registry can be found in {{compression-algos}}.  Adding a new compression key
...
stop.  See also the security note in {{security-considerations}} regarding differences
...
Hash -
: a hash function in which hLen denotes the length in octets of
...
2. Using the list in {{version-three-sig}}, produce an ASN.1 DER value for
...
are not present, then they are assumed to be \[ZIP(1), Uncompressed(0)\].
...
algorithm.  These are marked in {{pubkey-algos}} as "reserved for".
...
image format types, and algorithms described in {{constants}} all reserve
...
As described in the non-normative explanation in {{seipd}}, the
